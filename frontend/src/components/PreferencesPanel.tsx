...
export function PreferencesPanel({ kind, scope = "personal", displayName = "" }: { kind: "tags" | "memories"; scope?: "personal" | "shared"; displayName?: string }) {
  ...
  return <section className="settings-panel preference-panel">
    <form onSubmit={(e) => { e.preventDefault(); void run(async () => {
      ...
      <label>{kind === "tags" ? "タグ名" : scope === "shared" ? "共通の注意事項" : `${displayName}向けの注意事項`}
        <textarea required maxLength={kind === "tags" ? 60 : 500} value={value} onChange={(e) => setValue(e.target.value)} />
      </label>
    ...
    <div className="preference-list">{rows.map((row) => <div key={row.id}>
      ...
      {"name" in row && <button className="icon-button secondary" title="タグ名を編集" aria-label="タグ名を編集" onClick={() => { setEditing(row.id); setValue(row.name); }}><Pencil size={16} /></button>}
